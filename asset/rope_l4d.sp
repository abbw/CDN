...
{
	name = "绳索",
	author = " pan xiao hai,Neptune",
	description = "汉化Neptune",
	version = "1.0",
...
{ 	 
	...
	RegConsoleCmd("sm_rope", sm_rope,"绳索插件");
	
	g_iVelocity = FindSendPropOffs("CBasePlayer", "m_vecVelocity[0]");
	
	l4d_rope_damage = CreateConVar("l4d_rope_damage", "10", " 耐久度", FCVAR_PLUGIN);
 	l4d_rope_distance = CreateConVar("l4d_rope_distance", "900.0", "可用范围", FCVAR_PLUGIN);
	l4d_rope_adminonly=CreateConVar("l4d_rope_adminonly", "1", "1=只有管理员能使用,0=任何人可以使用", FCVAR_PLUGIN);
 
	l4d_rope_drop_from_witch = CreateConVar("l4d_rope_drop_from_witch", "50.0", "杀死witch获得绳索的几率", FCVAR_PLUGIN);
 	l4d_rope_drop_from_tank = CreateConVar("l4d_rope_drop_from_tank", "50.0", "杀死tank获得绳索的几率", FCVAR_PLUGIN);
...
public Action:sm_rope(client,args)
{
	...
	{
		if(GetConVarInt(l4d_rope_adminonly)==1 && GetUserFlagBits(client)==0)
		{
			PrintToChatAll("\x03只有\x04管理员\x03才能使用指令获得绳索哦！^_^");
			return;
		}
		else
...
{ 
	...
	if(witchid>0 && attacker>0)
	{
		...
		{
			PrintToChatAll("\x04%N\x03杀死\x04witch\x03获得绳索", attacker);
			GiveRope(attacker);
		}
	}
...
{ 
	...
	if(victim>0 && attacker>0)
	{
		...
		{
			PrintToChatAll("\x04%N\x03杀死\x04tank\x03获得绳索", attacker);
			GiveRope(attacker);
		}
	} 
}
...
EnableRope(client)
{
	...
	PrintToChatAll("\x04%N\x03启动了绳索,绳索获得方法:杀死\x04tank\x03或\x04witch\x03,或者输入\x04!rope",client);
	PrintToChatAll("\x03插件汉化重置者:\x04Neptune\x03,steamid:\x04Neptune\x03,原作者:\x04pan xiao hai",client);
	 
	PrintToChat(client, "\x03 您已经启动了绳索，使用\x04瞄准健\x03使用绳子");
	PrintToChat(client, "\x03 按下\x04跳跃\x03后按\x04Shift\x03上升,\x04Ctrl\x03下降");
